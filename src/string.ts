...
export function isShortLivingBranch(branchLike?: BranchLike): branchLike is ShortLivingBranch {
  return (
    isBranch(branchLike) && !branchLike.isMain && (branchLike as ShortLivingBranch).type === 'SHORT'
  );
}

export function isLongLivingBranch(branchLike?: BranchLike): branchLike is LongLivingBranch {
  return (
    isBranch(branchLike) && !branchLike.isMain && (branchLike as LongLivingBranch).type === 'LONG'
  );
}

export function hasNoPRStatus(branchLike?: BranchLike) {
  return Boolean(
    (isShortLivingBranch(branchLike) || isPullRequest(branchLike)) && !branchLike.status
  );
}
...
export function isPullRequestOrShortLivingBranch(
  branchLike?: BranchLike
): branchLike is PullRequest | ShortLivingBranch {
  return isPullRequest(branchLike) || isShortLivingBranch(branchLike);
}
...
export function getBranchLikeBaseBranch(branchLike: BranchLike) {
  return isPullRequest(branchLike) ? branchLike.branch : branchLike.name;
}
...
export function isTaskOnSameBranch(
...
export function sortBranchesAsTree(branchLikes: BranchLike[]) {
  ...
  // finally all orphan pull requests and branches
  result.push(
    ...sortBy(
  ...
  function getPullRequests(base: string) {
    return sortBy(
      pullRequests.filter((pr) => pr.base === base),
      (pullRequest) => pullRequest.key
    );
  }
}
...
export function getBranchLikeQuery(
  ...
  } else if (withMainBranch && isMainBranch(branchLike)) {
    return { branch: branchLike.name };
  }
  return {};
}
...
const DEFAULT_SHA_LENGTH = 8;

export function getReadableSha(sha: string, length = DEFAULT_SHA_LENGTH): string {
  return sha.length > length ? sha.substring(0, length) : sha;
}
    
export default function applyCodeDiff(element: Element | null) {
  if (element === null) {
    return;
  }
  ...
  codeExamples.forEach(({ noncompliant, compliant }) => {
    ...
    const [markedNonCompliant, markedCompliantCode] = performCodeDiff(
...
function performCodeDiff(compliant: string, nonCompliant: string) {
...
function replaceInDom(current: Element, code: string) {
  ...
  markedCode.classList.add('code-diff');
...
export function slugify(text: string) {
  return latinize(text.trim().toLowerCase())
    ...
    .replace(/[Â·/_,:;]/g, '-') // Replace special chars with dash
...
export function limitString(str: string, limit = 30): string {
